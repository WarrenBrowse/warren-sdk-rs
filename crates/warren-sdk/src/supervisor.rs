...
pub(crate) fn multihop_dial(
    ...
    // The dialed hop's other address family, when the directory published one:
    // on a network with no IPv4 route it is the only address that can work, and
    // on every other network it is ignored.
