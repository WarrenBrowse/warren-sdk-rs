...
leaves with `warren-bolthole remove-peer <label>`, which drops its session and
...
- **`Endpoint`** must be an address the device can reach. It is what
  `WARREN_BOLTHOLE_ENDPOINT` was set to when the peer was generated, so a peer
...
- **`MTU = 1280`** is what the gateway recommends by default
  (`WARREN_BOLTHOLE_CLIENT_MTU`); `/status` reports the live inner budget and the
...
```sh
warren-bolthole show peer2          # the text a file import wants
warren-bolthole show peer2 --qr     # the same thing as a QR, for a camera
```

In a container, put `docker compose exec warren-bolthole` in front of it.
...
The official WireGuard app on both platforms scans a QR: `warren-bolthole show
...
`WARREN_BOLTHOLE_FWMARK=<n>` on Linux with the peer's `FwMark = <n>` and an
`ip rule add fwmark <n> lookup main` installed once by root, or
`WARREN_BOLTHOLE_BIND_IF` on macOS and Windows.

## When something does not work

| symptom | what it usually is |
|---|---|
| no handshake at all | the device cannot reach `Endpoint`: wrong address, a firewall on the gateway host, or the gateway bound to loopback because `WARREN_BOLTHOLE_LAN` is unset |
...
| a device that was fine is refused after a clock jump | its handshake timestamps went backwards. `warren-bolthole reset-peer <label>` clears that guard for one peer |
