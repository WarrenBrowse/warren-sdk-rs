...
| `warren-net` | net | `PacketSink` seam + QUIC plane + smoltcp userspace netstack (TCP/UDP, dual-stack IPv6) + SOCKS5/HTTP proxy (CONNECT tunnels, plain-HTTP forwarding) + DNS-over-tunnel + NAT-PMP port-forwarding (client + inbound listen/relay) + killswitch levels; per-OS privileged TUN backend feature-gated (todo) | done (proxy); TUN todo |
...
| Handshake | HPKE-sealed multihop session: sealed control `IpRequest` / `IpAssign` (control `/v3`, marker `0xC0 0x03`, DAITA capability echo); admission on an anonymous v7 session token (`IpRequestV7`, walked past a serial the exit refuses) or, per `SessionAdmission`, by the wallet's proof-of-possession (`pop`); relay-leg auth signs the exported TLS channel binding (`sign_relay_auth`), no mutual-TLS client certificate |
