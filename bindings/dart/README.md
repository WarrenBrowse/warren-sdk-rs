...
- The pure identity surface (ss58 encode/decode, address-from-mnemonic) is bound
  by a small hand-written FFI (`lib/src/identity.dart`) and is PROVEN: `dart test`
  replays the shared `vectors/identity.json` byte-for-byte through the real cdylib
  (`test/vectors_test.dart`, all green). This is the cross-language wire-compat
  contract, satisfied on the Dart side for these functions.
- The full async surface (client construction, account/discovery, the proxy
  lifecycle, DAITA, port forwarding) is generated by `tool/generate.sh` but the
  current generator (`uniffi-bindgen-dart` 0.1.3) emits glue that crashes at the
  FFI boundary (see below), so it is not yet usable. The hand-written layer covers
  the pure functions in the meantime.
